...
class Residents {
  ...
  static async getResident(req, res) {
    ...
    */
      ...
      const data = await prisma.residents.findFirst({
        where: whereClause,
        select: {
          id: true,
          email: true,
      ...
      return res.json(
        ApiResponse(
          "Successfully fetched specific resident",
          data,
          StatusCodes.OK
        )
      );
  ...
  */
    ...
    try {
      ...
      let estateData = await prisma.residents.findFirst({
        where: {
      ...
      let data = [];
      try {
        data = await prisma.residents.create({
          data: {
            ...newObj,
          },
        });
  ...
  */
    ...
    try {
      ...
      let feedbackData = await prisma.residents.findFirst({
        where: {
          estate_id: estateData.id,
          resident_id: residentData.id,
      ...
      // CREATES NEW ENTRY
      let data = await prisma.residents.update({
        where: {
          id: feedbackData.id,
        },
        data: {
  ...
  // PATCH: /api/residents/:id (TO UPDATE)
  static async deleteResident(req, res) {
    ...
    */
      ...
      // Delete instance
      const data = await prisma.residents.update({
        where: {
          id: estateId,
        },
        data: {
      ...
      return res.json(
        ApiResponse("Successfully deleted resident", data, StatusCodes.OK)
      );
    } catch (error) {
      // Parsing error
      // error.message is an error object property
